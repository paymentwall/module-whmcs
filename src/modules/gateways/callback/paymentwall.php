    ...
    if ($invoiceid) {
        ...
        if ($invoiceid) {
            ...
            WHERE tblinvoices.id='" . (int)$relId . "' 
                AND (tblinvoiceitems.type='AddFunds'  OR (tblinvoiceitems.type='' AND tblinvoiceitems.relid=0)) AND ". ($requestData['type'] == 2 ? "tblinvoices.status='Paid'" : "tblinvoices.status='Unpaid'") . "
            ORDER BY tblinvoices.id ASC";
            $result = full_query($query);
            $data = mysql_fetch_assoc($result);
            $invoiceid = $data['id'];
